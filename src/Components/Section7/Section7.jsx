...
//     axios
//       .get(
//         `//products/getallproducts.php?nocache=${Date.now()}`
//       )
//       .then((res) => {
//         if (res.data.status) {
//           const formatted = res.data.data
//             .filter((item) => item.category_key === "disposable") // ✅ فلترة liquids فقط
//             .map((item) => ({
//               id: item.product_id,
//               name: item.name_en,
    ...
    axios
      ...
      .then((res) => {
        if (res.data.status) {
          ...
          const formatted = res.data.data
            .filter((item) => item.data?.category_en === "disposable")
            .map((item) => {

              const firstVariant = item.disposable?.[0];
              ...
              return {
                ...
                flavorCount: item.disposable?.length || 0,

                variants: item.disposable || [],
              };
            });
    ...
    <>
      <div className="flex justify-between">
        ...
        <h2 className="text-[42px] mb-8 text-white font-bold">
          {isArabic ? "منتجات السوائل" : "Our disposable Products"}
        </h2>
